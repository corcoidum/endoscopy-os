...
from tests.conftest import TEST_ORIGIN, SeededIdentity, login_admin
...
def test_change_keeps_same_appointment_and_records_revision(
    ...
    booked = _book(
        client, csrf_token, patient_id, service_date="2026-09-10", start_time="09:00"
    )
...
def test_stale_or_conflicting_change_leaves_appointment_unchanged(
    ...
    assert _book(
        ...
        service_date="2026-09-10", start_time="09:00", procedures=COLON,
    ).status_code == 201
    second = _book(
        client, csrf_token, patient_id, service_date="2026-09-10", start_time="10:00"
    )
...
def test_cancel_releases_capacity_and_blocks_further_changes(
    ...
    for start_time in ("09:00", "09:30", "10:00", "10:30", "11:00"):
        response = _book(
            client, csrf_token, patient_id,
            service_date="2026-09-10", start_time=start_time,
        )
        assert response.status_code == 201, response.text
        booked_ids.append(response.json()["id"])

    assert _book(
        client, csrf_token, patient_id, service_date="2026-09-10", start_time="11:30"
...
def test_no_show_requires_start_time_to_pass(
    ...
    future = _book(
        client, csrf_token, patient_id, service_date="2030-01-03", start_time="09:00"
    )
    assert past.status_code == future.status_code == 201

    too_early = client.post(
        f"/api/appointments/{future.json()['id']}/no-show",
        headers=_headers(csrf_token),
    ...
    recorded = client.post(
        f"/api/appointments/{past.json()['id']}/no-show",
        headers=_headers(csrf_token),
...
def test_closed_override_blocks_booking_until_revoked(
    ...
    decision = _approved_override(
        client, csrf_token, service_date="2026-09-17", rule_type="CLOSED"
    )
    assert decision["override"]["status"] == "APPROVED"
    assert decision["impacted_appointments"] == []

    closed = _availability(client, "2026-09-17", ["UPPER"])
    assert closed.status_code == 409
    assert closed.json()["code"] == "SCHEDULE_CLOSED"
    blocked = _book(
        client, csrf_token, patient_id, service_date="2026-09-17", start_time="09:00"
    )
    ...
    assert _availability(client, "2026-09-17", ["UPPER"]).status_code == 200
...
def test_operating_hours_override_limits_last_start_and_reports_impact(
    ...
    patient_id = _create_patient(client, csrf_token)
    late = _book(
        client, csrf_token, patient_id, service_date="2026-09-17", start_time="11:00"
    )
    ...
    availability = _availability(client, "2026-09-17", ["COLON"])
    ...
    too_late = _book(
        client, csrf_token, patient_id,
        service_date="2026-09-17", start_time="10:00", procedures=COLON,
    )
    ...
    policies = client.get(
        ...
        params={"start_date": "2026-09-16", "end_date": "2026-09-17"},
    )
...
def test_capacity_override_is_superseded_by_newer_approval(
    ...
    first = _approved_override(
        client, csrf_token,
        service_date="2026-09-17", rule_type="CAPACITY", override_upper_capacity=1,
    )
    assert _book(
        client, csrf_token, patient_id, service_date="2026-09-17", start_time="09:00"
    ).status_code == 201
    over = _book(
        client, csrf_token, patient_id, service_date="2026-09-17", start_time="09:30"
    )
    ...
    second = _approved_override(
        client, csrf_token,
        service_date="2026-09-17", rule_type="CAPACITY", override_upper_capacity=2,
    )
    overrides = client.get(
        "/api/schedule/overrides",
        params={"start_date": "2026-09-17", "end_date": "2026-09-17"},
    ).json()
...
def test_afternoon_exception_requires_allowed_date_and_different_confirmer(
    ...
    _approved_override(
        client, csrf_token, service_date="2026-09-10", rule_type="AFTERNOON_ALLOW"
    )
    availability = _availability(
        client, "2026-09-10", ["UPPER"], booking_bucket="AFTERNOON_EXCEPTION"
    )
    assert availability.status_code == 200, availability.text
...
def test_changing_confirmed_afternoon_exception_requires_reconfirmation(
    ...
    csrf_token = _login_admin(client)
    patient_id = _create_patient(client, csrf_token)
    for service_date in ("2026-09-10", "2026-09-11"):
        _approved_override(
            client, csrf_token, service_date=service_date, rule_type="AFTERNOON_ALLOW"
        )
    booked = _book(
        client, csrf_token, patient_id,
        service_date="2026-09-10", start_time="14:00",
    ...
    moved = client.patch(
        appointment_url,
        headers=_headers(second_csrf),
        json={"row_version": 2, "reason": "날짜 변경", "service_date": "2026-09-11"},
    )
